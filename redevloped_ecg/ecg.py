...
logger.info("Initializing Hybrid CNN + Naive Bayes + MobileNet model...")

try:
    base_model = MobileNetV2(
        weights='imagenet',     # attempts to download pretrained weights (may cause HTTP errors)
        include_top=False,
        input_shape=(224, 224, 3)
    )
    logger.info("MobileNet component loaded successfully with 'imagenet' weights.")
except Exception as e:
    # Print full traceback to help identify if this was an HTTP 404 or other error
    logger.warning("Exception while loading MobileNet imagenet weights: %s", e)
    logger.warning("Full traceback follows:")
    traceback.print_exc()

    # Fallback to random-initialized weights so script can continue
    logger.info("Falling back to MobileNet with weights=None (randomly initialized).")
    base_model = MobileNetV2(
        weights=None,
        include_top=False,
        input_shape=(224, 224, 3)
...
logger.info("MobileNet component initialized (may be untrained if weights=None).")
...
def create_welcome_html():
    ...
    <head>
        <style>
            ...
            .ecg-animation {
              height: 60px;
             /* Use a compact CSS gradient instead of a huge/inlined SVG data URI */
              background: linear-gradient(90deg, rgba(76,175,80,0.15) 0%, rgba(255,193,7,0.15) 50%, rgba(244,67,54,0.15) 100%);
